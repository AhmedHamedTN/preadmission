        ...
        setTimeout(function () {
            ...
            if (connection.request.url.endsWith('/api/authenticate') && connection.request.method === http_1.RequestMethod.Post) {
                ...
                var filteredUsers = users.filter(function (user) {
                    return user.clinicalid === params_1.clinicalid && user.password === params_1.password;
                });
                if (filteredUsers.length) {
                    ...
                    connection.mockError(new Error('l\'ID ou le Mot de passe est incorrect'));
                }
                return;
            }
            // get users
            ...
            if (connection.request.url.endsWith('/api/users') && connection.request.method === http_1.RequestMethod.Post) {
                ...
                // validation
                var duplicateUser = users.filter(function (user) { return user.clinicalid === newUser_1.clinicalid; }).length;
                if (duplicateUser) {
                    return connection.mockError(new Error('Clinical ID "' + newUser_1.clinicalid + '" is already taken'));
                }
                // save new user
